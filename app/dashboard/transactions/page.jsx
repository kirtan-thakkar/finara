          ...
          <Card className="border-slate-200/60 dark:border-slate-700/50 shadow-sm mb-6 p-0 gap-0">
            <CardContent className="p-4">
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <Select value={filterRecurring || "ALL"} onValueChange={(val) => setFilterRecurring(val === "ALL" ? "" : val)}>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-b border-slate-200 dark:border-slate-700">
                      ...
                      <TableHead className="py-3 px-4 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide">Category</TableHead>
                      <TableHead className="py-3 px-4 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide">Type</TableHead>
                      <TableHead className="py-3 px-4 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide">Status</TableHead>
                  ...
                  <TableBody>
                    {sortedTransactions.map((t) => (
                      <TableRow key={t._id} className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group ${selectedIds.includes(t._id) ? "bg-emerald-50/50 dark:bg-emerald-900/10" : ""}`}>
                        ...
                        <TableCell className="py-3.5 px-4">
                          <Badge variant="secondary" className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-medium">{t.category}</Badge>
                        </TableCell>
                        <TableCell className="py-3.5 px-4">
                          ...
                          }>{t.type === "INCOME" ? "Income" : "Expense"}</Badge>
                        </TableCell>
                        <TableCell className="py-3.5 px-4">
                          <Badge variant="secondary" className={
                        ...
                        <TableCell className={`py-3.5 px-4 text-sm font-semibold text-right tabular-nums ${
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-40">
                              ...
                              <DropdownMenuItem onClick={() => handleDuplicate(t._id)} disabled={duplicating === t._id}>
                                {duplicating === t._id ? <Loader2 className="w-3.5 h-3.5 mr-2 animate-spin" /> : <Copy className="w-3.5 h-3.5 mr-2" />} Duplicate
                              </DropdownMenuItem>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem onClick={() => handleDelete(t._id)} disabled={deleting === t._id}
                                className="text-red-600 dark:text-red-400 focus:text-red-600 dark:focus:text-red-400">
                                {deleting === t._id ? <Loader2 className="w-3.5 h-3.5 mr-2 animate-spin" /> : <Trash2 className="w-3.5 h-3.5 mr-2" />} Delete
                              </DropdownMenuItem>
                            </DropdownMenuContent>
                          </DropdownMenu>
                        </TableCell>
        ...
        <Dialog open={sheetOpen} onOpenChange={setSheetOpen}>
          <DialogContent className={`sm:max-w-lg max-h-[90vh] overflow-y-auto ${generalSans.className}`}>
            ...
            <form onSubmit={isFormSubmit} className="flex flex-col gap-5">
              ...
              {/* Type toggle */}
              <div>
                <Label className="mb-1.5 block text-sm font-medium text-black">Type</Label>
                <div className="grid grid-cols-2 gap-2">
                  <Button type="button" variant="outline" onClick={() => setFormData({ ...formData, type: "INCOME" })}
                    className={`py-2.5 rounded-lg transition-colors ${formData.type === "INCOME"
                  ...
                  <Button type="button" variant="outline" onClick={() => setFormData({ ...formData, type: "EXPENSE" })}
                    className={`py-2.5 rounded-lg transition-colors ${formData.type === "EXPENSE"
              ...
              {/* Title */}
              <div>
                <Label htmlFor="title" className="mb-1.5 block text-sm font-medium text-black">Title *</Label>
              ...
              {/* Amount */}
              <div>
                <Label htmlFor="amount" className="mb-1.5 block text-sm font-medium text-black">Amount *</Label>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <Label htmlFor="date" className="mb-1.5 block text-sm font-medium text-black">Date *</Label>
              ...
              {/* Description */}
              <div>
                <Label htmlFor="description" className="mb-1.5 block text-sm font-medium text-black">Description</Label>
              ...
              <div>
                <div className="flex items-center gap-3">
                  ...
                  />
                  <Label className="cursor-pointer text-sm font-medium text-black">Recurring transaction</Label>
                </div>
