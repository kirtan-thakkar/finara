...
export default function Sidebar({ isCollapsed = false }) {
  ...
  return (
    <div className={`fixed left-0 top-0 h-full bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'} z-40`}>
      <div className="flex flex-col h-full">
        <div className="p-6 border-b border-gray-200 dark:border-gray-800">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-gray-900 dark:bg-gray-100 rounded-lg flex items-center justify-center">
              <DollarSign className="w-4 h-4 text-white dark:text-gray-900" />
            </div>
            {!isCollapsed && (
              <span className="text-lg font-medium text-gray-900 dark:text-gray-100">FinanceTracker</span>
            )}
          </div>
        </div>
            ...
            {bottomItems.map((item, index) => {
              ...
              return (
                <li key={index}>
                  <a
                    href={item.href}
                    className="flex items-center gap-3 px-3 py-2.5 rounded-md text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors text-sm"
                  >
                    <Icon className="w-4 h-4 flex-shrink-0" />
                    {!isCollapsed && <span className="font-medium">{item.label}</span>}
                  </a>
